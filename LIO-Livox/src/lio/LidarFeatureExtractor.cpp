...
void LidarFeatureExtractor::detectFeaturePoint(pcl::PointCloud<PointType>::Ptr& cloud,
  ...
  int cloudAngle[20000]; // 当相邻点与到当前点的视线夹角小于15度时产生标记
  ...
  for (int i = 5; i < cloudSize - 5; i ++ ) {
    ...
    double angle_last = (pt_last-pt_cur).dot(pt_cur) / ((pt_last-pt_cur).norm()*pt_cur.norm()); // 计算二者之间夹角余弦值 wjc
    double angle_next = (pt_next-pt_cur).dot(pt_cur) / ((pt_next-pt_cur).norm()*pt_cur.norm()); // 计算二者之间夹角余弦值 wjc
 
    if (dis > thDistanceFaraway || (fabs(angle_last) > 0.966 && fabs(angle_next) > 0.966)) {
      thNumCurvSize = 2; // 距离远或者小于14度是这个阈值 wjc
    } else {
      thNumCurvSize = 3;
    }
